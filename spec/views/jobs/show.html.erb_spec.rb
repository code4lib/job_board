...
RSpec.describe "jobs/show", type: :view do
  ...
  it 'contains metadata about when the job was created' do
    ...
    expect(rendered).to have_content(/Created:\s+#{view.l(@job.created_at.to_date)}/)
  ...
  describe 'for a published job' do
    ...
    it 'contains metadata about when the job was published' do
      ...
      expect(rendered).to have_content 'Metadata'
      expect(rendered).to have_content(/Created:\s+#{view.l(@job.created_at.to_date)}/)
      expect(rendered).to have_content(/Last updated:\s+#{view.l(@job.updated_at)}/)
      expect(rendered).to have_content(/Published:\s+#{view.l(@job.published_at)}/)
