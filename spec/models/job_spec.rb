...
RSpec.describe Job, type: :model do
  ...
  describe '#publish!' do
    before do
      subject.update(title: 'x', description: 'y', job_type: :full_time, origin: 'jobs.code4lib.org')
    end
    ...
    it 'sends email notifications' do
      expect { subject.publish! }.to have_enqueued_job(ActionMailer::MailDeliveryJob)
    end
    ...
    context 'a harvested job' do
      ...
      it 'does not send email notifications' do
        expect { subject.publish! }.not_to have_enqueued_job(ActionMailer::MailDeliveryJob)
      end
    end
  end
end
